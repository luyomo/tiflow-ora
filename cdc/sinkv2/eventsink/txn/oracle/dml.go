...
func prepareUpdate(quoteTable string, preCols, cols []*model.Column, forceReplicate bool) (string, []interface{}) {
	log.Info("oracle prepareUpdate")
	quoteTable = strings.ReplaceAll(quoteTable, "`", "")
	...
	for _, col := range cols {
		...
		columnNames = append(columnNames, strings.ReplaceAll(col.Name, "`", "") )
...
func prepareReplace(
	...
	log.Info("oracle prepareReplace")
	quoteTable = strings.ReplaceAll(quoteTable, "`", "")
	var builder strings.Builder
	columnNames := make([]string, 0, len(cols))
	...
	log.Info("column names", zap.String("columns", fmt.Sprintf("%#v", columnNames)))
