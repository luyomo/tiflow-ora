...
func NewOracleBackends(
	...
	cfg.IsTiDB, err = poracle.CheckIsTiDB(ctx, db)
	if err != nil {
		return nil, err
	}
	...
	backends := make([]*oracleBackend, 0, cfg.WorkerCount)
	for i := 0; i < cfg.WorkerCount; i++ {
		backends = append(backends, &oracleBackend{
			workerID:    i,
...
func (s *oracleBackend) OnTxnEvent(event *eventsink.TxnCallbackableEvent) (needFlush bool) {
...
func (s *oracleBackend) Flush(ctx context.Context) (err error) {
	if s.rows == 0 {
		return
	}
...
func (s *oracleBackend) Close() (err error) {
	if s.db != nil {
		err = s.db.Close()
		s.db = nil
	}
...
func (s *oracleBackend) MaxFlushInterval() time.Duration {
...
func (s *oracleBackend) groupRowsByType(
...
func (s *oracleBackend) batchSingleTxnDmls(
	event *eventsink.TxnCallbackableEvent,
	tableInfo *timodel.TableInfo,
	translateToInsert bool,
) (sqls []string, values [][]interface{}) {
	insertRows, updateRows, deleteRows := s.groupRowsByType(event, tableInfo, !translateToInsert)
	log.Info("batchSingleTxnDmls", zap.String("insertRows", fmt.Sprintf("%#v", insertRows)), zap.String("updateRows", fmt.Sprintf("%#v", updateRows)) , zap.String("deleteRows", fmt.Sprintf("%#v", deleteRows)) )
...
func (s *oracleBackend) genUpdateSQL(rows ...*sqlmodel.RowChange) ([]string, [][]interface{}) {
	size, count := 0, 0
	for _, r := range rows {
...
func (s *oracleBackend) prepareDMLs() *preparedDMLs {
	...
	for _, event := range s.events {
		...
		for _, row := range event.Event.Rows {
                        log.Info("oracle loop in the event.Event.Rows")
			var query string
			var args []interface{}
...
func (s *oracleBackend) execDMLWithMaxRetries(pctx context.Context, dmls *preparedDMLs) error {
	...
	return retry.Do(pctx, func() error {
		...
		err := s.statistics.RecordBatchExecution(func() (int, error) {
			...
			log.Info("oracle dmls", zap.String("dmls", fmt.Sprintf("%#v", dmls.sqls)))
			for i, query := range dmls.sqls {
			        log.Info("oracle dml", zap.String("query", query), zap.String("values", fmt.Sprintf("%#v", dmls.values)))
				args := dmls.values[i]
...
// Only for testing.
func (s *oracleBackend) setDMLMaxRetry(maxRetry uint64) {
	s.dmlMaxRetry = maxRetry
}

// setWriteSource sets write source for the transaction.
func (s *oracleBackend) setWriteSource(ctx context.Context, txn *sql.Tx) error {
