...
func (c *Config) Apply(
	...
	log.Warn("oracle url", zap.String("url", fmt.Sprintf("%#v", *sinkURI) ) )
	if sinkURI == nil {
		return cerror.ErrMySQLConnectionError.GenWithStack("fail to open MySQL sink, empty SinkURI")
	}

	scheme := strings.ToLower(sinkURI.Scheme)
	if !sink.IsOracleCompatibleScheme(scheme) {
		return cerror.ErrMySQLConnectionError.GenWithStack("can't create oracle sink with unsupported scheme: %s", scheme)
	}
	query := sinkURI.Query()
