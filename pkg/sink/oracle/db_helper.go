...
func CreateOracleDBConn(ctx context.Context, dsnStr string) (*sql.DB, error) {
	log.Info("oracle connection string", zap.String("oracle", dsnStr))
...
//	if err != nil {
//	        log.Info("querySQLMode", zap.String("sql_mode", "test"))
...
//	        log.Info("checkCharsetSupport", zap.String("sql_mode", "test"))
...
//	if err != nil && err != sql.ErrNoRows {
//	        log.Info("checkTiDBVariable", zap.String("sql_mode", "test"))
...
func GetTestDB(ctx context.Context, dbConfig *dmysql.Config, dbConnFactory Factory) (*sql.DB, error) {
	...
	if dbConnFactory == nil {
		dbConnFactory = CreateOracleDBConn
	}
...
func CheckIsTiDB(ctx context.Context, db *sql.DB) (bool, error) {
	var oracleVer string
	// check if downstream is TiDB
	row := db.QueryRowContext(ctx, "SELECT BANNER_FULL FROM V$VERSION")
	err := row.Scan(&oracleVer)
	if err != nil {
		log.Error("check oracle version error", zap.Error(err))
	...
	log.Info("Oracle version:", zap.String("version", oracleVer))
	return true, nil
}
